...
class Main:
    ...
    def run(self, genomes, config) -> None:
        ...
        while var_boolRunning:
            ...
            for index, bird in enumerate(self.var_clssBird.var_listItens):
                # Verificando se o "passaro colidiu com algum cano"
                var_boolPassed = self.var_clssPipe.collide(bird)

                # Caso o "passaro tenha colidido com algum cano"
                if any(var_boolPassed):
                    # "Apagando o passaro"
                ...
                # Caso o "passaro NÃO tenha colidido com algum cano" e ela tenha ultrapassado o cano
                if not any(var_boolPassed) and bird['x'] > (
                    self.var_clssPipe.var_listItens[self.var_intPipeIndex]['pipe_base'].x + (
                        self.var_clssPipe.WIDTH - self.var_clssPipe.VELOCITY
                    )):
                    # Pontuando "o passaro" por "ter passado por um cano"
                    if var_boolIAPlay:
            ...
            # Aumentando a pontuação, Caso "algum passaro tenha passado por um cano"
            if var_boolPointed:
...
if __name__ == '__main__':
    ...
    try:
        # Caso IA jogando
        if var_boolIAPlay: population.run(Main().run, 100)
        # Caso Player jogando
